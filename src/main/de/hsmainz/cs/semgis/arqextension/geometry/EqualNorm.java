...
public class EqualNorm extends FunctionBase2 {
	...
	public NodeValue exec(NodeValue v1, NodeValue v2) {
		...
		if(wrapper1 instanceof GeometryWrapper && wrapper2 instanceof GeometryWrapper) {
			return NodeValue.makeBoolean(((GeometryWrapper)wrapper1).getXYGeometry().equalsNorm(((GeometryWrapper)wrapper2).getXYGeometry()));
			...
			if(wrapper1 instanceof CoverageWrapper) {
				...
				return NodeValue.makeBoolean(LiteralUtils.toGeometry(bbox1.getBounds()).equalsNorm((geom)));
			}else {
				GridCoverage2D raster=((CoverageWrapper)wrapper2).getXYGeometry();
				Rectangle2D bbox1 = raster.getEnvelope2D().getBounds2D();
				Geometry geom=((GeometryWrapper)wrapper1).getXYGeometry();
				return NodeValue.makeBoolean(geom.equalsNorm(LiteralUtils.toGeometry(bbox1.getBounds())));				
			}
		}
	}
