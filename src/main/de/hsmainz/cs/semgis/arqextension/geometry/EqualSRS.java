...
public class EqualSRS extends FunctionBase2 {
	...
	public NodeValue exec(NodeValue v1, NodeValue v2) {
		...
		if(wrapper1 instanceof GeometryWrapper && wrapper2 instanceof GeometryWrapper) {
	        ...
	        return NodeValue.makeBoolean(LiteralUtils.toGeometry(bbox1.getBounds()).getSRID()==LiteralUtils.toGeometry(bbox2.getBounds()).getSRID());			
		}else {
			if(wrapper1 instanceof CoverageWrapper) {
				GridCoverage2D raster=((CoverageWrapper)wrapper1).getXYGeometry();
				Rectangle2D bbox1 = raster.getEnvelope2D().getBounds2D();
				Geometry geom=((GeometryWrapper)wrapper2).getXYGeometry();
				return NodeValue.makeBoolean(LiteralUtils.toGeometry(bbox1.getBounds()).getSRID()==geom.getSRID());
			}else {
				GridCoverage2D raster=((CoverageWrapper)wrapper2).getXYGeometry();
				Rectangle2D bbox1 = raster.getEnvelope2D().getBounds2D();
				Geometry geom=((GeometryWrapper)wrapper1).getXYGeometry();
				return NodeValue.makeBoolean(geom.getSRID()==(LiteralUtils.toGeometry(bbox1.getBounds())).getSRID());				
			}
		}
	}
